# -*- coding: utf-8 -*-
r"""SD 侧 bValid 专项：主链 buildBasis 段输出 (Tuv, tValid) / (Buv, bValid)。
输出：sd/validation/sd_bvalid.exr（R/G/B=Buv*0.5+0.5 A=bValid）
"""
import json
import math
import os
...
def main():
    ...
    with SDAPI.undo_group('aniso_pp bvalid: build'):
        ...
        pp.setPosition(float2(27000.0, -2600.0))
    ...
    s_pos, s_nrm, s_mask = sample(0), sample(1), sample(2)
    texel = 1.0 / 2048.0

    coverage = em.step(em.c_f1(0.5), em.sw1(s_mask, 0))
    ...
    tValid = em.mul(em.mul(em.sw1(tuN, 3), em.sw1(dPdqx, 3)),
                    em.step(em.c_f1(1e-16), projLen2))
    handed = em.dot3(em.cross3(N0, Tuv), dPdv)
    bValid = em.mul(em.mul(em.step(em.c_f1(1e-12), em.abs_f1(handed)),
                           em.sw1(dPdqy, 3)), tValid)
    packed = em.v4_from_f3(
        em.add(em.mulscalar(Buv := em.mulscalar(
            em.cross3(N0, Tuv), em.pick_sign(handed)), em.c_f1(0.5)),
            em.bc_f3(em.c_f1(0.5))), bValid)
    fg.setOutputNode(packed.node, True)

    meta = {'done': True}
    compute_and_save(graph, pp, os.path.join(VAL_DIR, 'sd_bvalid.exr'))
    print('[DONE]', meta)
